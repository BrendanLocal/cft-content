...
export default function App({ file, href, children}) {
  ...
  const editUrlPrefix = '/net-negative-school?session=';
  const sharingUrlPrefix = '/net-negative-school-share?session=';
  const [editUrl, setEditUrl] = React.useState('/net-negative-school');
  const [sharingUrl, setSharingUrl] = React.useState('/net-negative-school-share');
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      setRegion(localStorage.getItem('schoolRegion'));
      setDuration(Number(localStorage.getItem('schoolDuration')));
      ...
      try {
        fetch(`/api/calc?sessionID=${sessionID}&type=net-negative-school`).then(async (response) => {
      ...
      <Container className="p-4 pt-5">
        <Row className="justify-content-center">
          <Col className="col-12 col-lg-10 pt-5 align-items-center my-4 pt-5">
            <h1 className="emphasis text-orange text-center bold tight-drop-light">School Net-Negative Calculator</h1>
          </Col>
        </Row>
        <Row className="justify-content-center">
        ...
        <Row className="justify-content-center ">
          <Col className="col-11 col-lg-10 align-items-center text-center p-3">
            <div className="bg-green p-4 innerShadow roundedBox">
              ...
              <FacebookShareButton url={hostname + editUrl} beforeOnClick={shareBeforeClick} quote={editingdata.shareSchool} className="mx-2">
                <FacebookIcon size={48} round />
              </FacebookShareButton>

              <TwitterShareButton url={hostname + editUrl} beforeOnClick={shareBeforeClick} title={editingdata.shareSchool} className="mx-2">
                <TwitterIcon size={48} round />
              </TwitterShareButton>

              <LinkedinShareButton url={hostname + editUrl} beforeOnClick={shareBeforeClick} summary={editingdata.shareSchool} className="mx-2">
                <LinkedinIcon size={48} round />
              </LinkedinShareButton>

              <EmailShareButton url={hostname + editUrl} beforeOnClick={shareBeforeClick} body={editingdata.shareSchool} className="mx-2">
